...
\TLV
   ...
   |cpu
      @0
         ...
         $pc[31:0] = >>1$reset        ? 32'b0 :
                     ...
                     //                   >>1$inc_pc;
                     >>3$valid_untaken_branch ? >>3$inc_pc :
                     !>>1$is_b_instr    ? >>1$inc_pc :
                                         >>1$br_target_pc;

         // Lab: Fetch
         $imem_rd_addr[4:0] = $pc[6:2];
      ...
      @2
         $src1_value[31:0] = >>1$rf_wr_valid && >>1$rf_wr_index == $rf_rd_index1 ? >>1$rf_wr_data :
                             >>2$rf_wr_valid && >>2$rf_wr_index == $rf_rd_index1 ? >>2$rf_wr_data :
                             $rf_rd_data1;
         $src2_value[31:0] = >>1$rf_wr_valid && >>1$rf_wr_index == $rf_rd_index2 ? >>1$rf_wr_data :
                             >>2$rf_wr_valid && >>2$rf_wr_index == $rf_rd_index2 ? >>2$rf_wr_data :
                             $rf_rd_data2;
         ...
         $untaken_branch = $is_blt && !($src1_value < $src2_value);
         $valid_untaken_branch = $untaken_branch && $valid;
         //$valid = ! $reset && ! >>1$valid_taken_branch && ! >>2$valid_taken_branch; // && ! $taken_branch;
         $valid = ! $reset && ! >>1$valid_untaken_branch && ! >>2$valid_untaken_branch; // && ! $taken_branch;
   ...
   m4+shell(@3, @0, @1, @3, @3, CHECK_VALUE, 75)  // (@viz, @imem, @rf_rd, @rf_wr, @rf_check, check_value, end_cycle)
